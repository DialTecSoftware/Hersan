...
namespace Hersan.UI.Ensamble
{
    public partial class frmCotizacionCorreo : Telerik.WinControls.UI.RadForm
    {
        ...
        {
            try {
                string Body = "Estimado Cliente:\n\nBuen día.\n\nDe acuerdo con su solicitud, adjunto encontrará la cotización: " + Id + ", la cual tiene vigencia de 30 días naturales a partir de la recepción del presente correo.\n\n";
                ...
                if (txtCorreo.Text.Length > 0 && Archivo != null) {
                    bool Flag = BaseWinBP.EnviarMail("Key.Solutions.Test@gmail.com", txtCorreo.Text, Correo, "", "Cotización", Body, 
                        "smtp.gmail.com", "Catcooptest", 587, Archivo, "Cotizacion.pdf",true);

                    if (Flag) 
                        RadMessageBox.Show("Cotización enviada correctamente", this.Text, MessageBoxButtons.OK, RadMessageIcon.Info);
                    else
